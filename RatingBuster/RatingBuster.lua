...
LastUpdate: $Date: 2010-03-26 14:08:49 +0000 (Fri, 26 Mar 2010) $
]]
...
RatingBuster.version = "1.4.8 (r"..gsub("$Revision: 276 $", "(%d+)", "%1")..")"
RatingBuster.date = gsub("$Date: 2010-03-26 14:08:49 +0000 (Fri, 26 Mar 2010) $", "^.-(%d%d%d%d%-%d%d%-%d%d).-$", "%1")
...
local summaryCalcData = {
	...
	-- "ADD_AP_MOD_SPELL_DMG" -- Warlock: Metamorphosis
	{
		...
		func = function(sum, sumType, link)
			...
			ap = ap + (summaryFunc["ARMOR"](sum, sumType, link) * 3/108)
			local mod = 1.21
			return ap * mod
		end,
	},
	...
	-- Feral Attack Power - FERAL_AP, AP, STR, AGI
	{
		...
		func = function(sum, sumType, link)
			-- Druid: Predatory Strikes 2,10: Increases 7%/14%/20% of any attack power on your equipped weapon.
			local p = 0
			if class == "HERO" and select(5, GetTalentInfo(2, 10)) > 0 and weaponItemEquipLoc[select(9, GetItemInfo(link))] then
				local r = select(5, GetTalentInfo(2, 10))
				if r == 1 then
					p = 0.07
				elseif r == 2 then
	...
	-- Spell Damage - SPELL_DMG, ADD_SPELL_DMG_MOD_STA, ADD_SPELL_DMG_MOD_INT, ADD_SPELL_DMG_MOD_SPI, ADD_SPELL_DMG_MOD_AP
	{
		...
		func = function(sum, sumType, link)
			local spellDmg = (sum["SPELL_DMG"] or 0)
			if StatLogic:GetStatMod("ADD_SPELL_DMG_MOD_STR") ~= 0 then
				spellDmg = spellDmg + (sum["STR"] * StatLogic:GetStatMod("ADD_SPELL_DMG_MOD_STR"))
			end
			...
			if StatLogic:GetStatMod("ADD_PET_STA_MOD_STA") ~= 0 then
				spellDmg = spellDmg + (sum["STA"] * StatLogic:GetStatMod("ADD_SPELL_DMG_MOD_PET_STA") * StatLogic:GetStatMod("ADD_PET_STA_MOD_STA"))
				spellDmg = spellDmg + (sum["INT"] * StatLogic:GetStatMod("ADD_SPELL_DMG_MOD_PET_INT") * StatLogic:GetStatMod("ADD_PET_INT_MOD_INT"))
			end
